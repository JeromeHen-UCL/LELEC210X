...
FREQ_SAMPLING = int(32e6/((4+1)*(159+1)))
VAL_MAX_ADC = 4096
...
def check_args(args: argparse.Namespace) -> None:
    ...
    if os.path.exists(args.recordings_dir):
        logger.info("Recordings directory %s/ exists.", args.recordings_dir)
    else:
        os.makedirs(args.recordings_dir)
        logger.info("Recordings directory %s/ created.", args.recordings_dir)
...
def main(args: argparse.Namespace):
    ...
    for i, frequency in enumerate(frequencies):
        ...
        logger.info("Frequency: %.2f Hz", frequency)
    ...
    if args.plot_response:
        ...
        plt.loglog(frequencies, means, "o")
        plt.fill_between(frequencies, means - stds, means + stds, color="blue", alpha=0.3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_recordings", action='store_true', default=None,
    ...
    parser.add_argument("--keep_recordings", action='store_true', default=None,
                        help="If the previous recording should be kept.")
    main_args = parser.parse_args()

    assert FREQ_STOP <= FREQ_SAMPLING / 2
